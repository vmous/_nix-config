...
;;;; Font (code)
;;
;; Code buffers use their own family and size (`jazzy/font-family-name-code'
;; and `jazzy/font-size-code', see `.emacs.d/environment.el') while everything
;; else keeps the `default' face font set in `.emacs'. The remap is
;; buffer-local and relative; the family is overridden and the size is set as an
;; absolute height (points * 10) so it is independent of the default face size.
;;
...
(defun jazzy/use-code-font ()
  "Remap the `default' face to the code font and size in the current buffer."
